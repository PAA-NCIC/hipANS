...
StackDeviceMemory::Stack::Stack(int d, size_t sz)
  ...
  HIP_VERIFY(hipMalloc(&alloc_, allocSize_));
  HIP_VERIFY(hipMemset(alloc_, 0, allocSize_));
  assert(alloc_);
...
void* StackDeviceMemory::Stack::getAlloc(
  ...
  if (size > sizeRemaining || type == AllocType::Permanent) {
    ...
    HIP_VERIFY(hipMalloc(&out, size));
    HIP_VERIFY(hipMemset(out, 0, size));
    assert(out);
