...
void compressBFloat16File(StackDeviceMemory& res, const std::string& inputFilePath, const std::string& tempFilePath, std::vector<uint32_t>& batchSizes, size_t& numElements, int probBits, uint32_t& maxCompressedSize, int& totalSize, std::vector<uint32_t>& compressedSize, int& numInBatch,hipStream_t stream) {
    auto compConfig = FloatCompressConfig(FloatType::kBFloat16, ANSCodecConfig(probBits), false, true);
    ...
    std::vector<FloatTypeInfo<FloatType::kBFloat16>::WordT> orig(numElements);
    ...
    auto inPtrs = std::vector<const void*>(batchSizes.size());
    {
      ...
      for (int i = 0; i < inPtrs.size(); ++i) {
        inPtrs[i] = (FloatTypeInfo<FloatType::kBFloat16>::WordT*)orig_dev.data() + curOffset;
        curOffset += batchSizes[i]*sizeof(FloatTypeInfo<FloatType::kBFloat16>::WordT);
      }
    }

    maxCompressedSize = getMaxFloatCompressedSize(FloatType::kBFloat16, maxSize);
...
void decompressBFloat16File(StackDeviceMemory& res, const std::string& tempFilePath, const std::string& outputFilePath, std::vector<uint32_t>& batchSizes, size_t& numElements, int probBits, uint32_t& maxCompressedSize, int& totalSize, std::vector<uint32_t>& compressedSize, int& numInBatch, hipStream_t stream) {
    ...
    // Decode data
    auto dec_dev = res.alloc<FloatTypeInfo<FloatType::kBFloat16>::WordT>(stream, totalSize);
    auto decPtrs = std::vector<void*>(batchSizes.size());
    {
      uint32_t curOffset = 0;
      for (int i = 0; i < encPtrs.size(); ++i) {
        decPtrs[i] = (FloatTypeInfo<FloatType::kBFloat16>::WordT*)dec_dev.data() + curOffset;
        curOffset += batchSizes[i]*sizeof(FloatTypeInfo<FloatType::kBFloat16>::WordT);
      }
    }
    ...
    auto decompConfig =
        FloatDecompressConfig(FloatType::kBFloat16, ANSCodecConfig(probBits), false, true);
    ...
    std::vector<FloatTypeInfo<FloatType::kBFloat16>::WordT> decompressedHost(batchSizes[0]);
...
int main(int argc, char* argv[]) {
    ...
    int totalSize;
    try{
    compressBFloat16File(res, argv[1], argv[2], batchSizes, numElements, probBits, maxCompressedSize, totalSize, compressedSize, numInBatch,stream);
    decompressBFloat16File(res, argv[2], argv[3], batchSizes, numElements, probBits, maxCompressedSize, totalSize, compressedSize, numInBatch,stream);
